...
interrupt(registers_t *reg)
{
	...
	case INT_SYS_GETPID:
		...
		run(current);
	case INT_SYS_NEWTHRD:
	        // Make a new thread assuming that the instruction pointer 
	        // will be stored at %eax. We follow the same format as INT_
	        // SYS_FORK 
	  current->p_registers.reg_eax=sys_newthread((void*)(current->p_registers.reg_eax));
	  run(current);
	case INT_SYS_FORK:
	...
	case INT_SYS_EXIT:
	  ...
	  if(current->waiting_proc!=NULL)
	    {
	      current->p_state=P_EMPTY;
	      current->waiting_proc->p_state=P_RUNNABLE;
	      current->waiting_proc->p_registers.reg_eax=current->p_registers.reg_eax;
	    }
	  if(current->waiting_proc==NULL) //Make sure if sys_wait called then code doesn't revert back to P_ZOMBIE
	...
	case INT_SYS_KILL:{
	  ...
	  if (kill_pid <= 0 || kill_pid >= NPROCS ||
	      kill_pid == current->p_pid|| 
	      proc_array[kill_pid].p_state == P_EMPTY)
	    current->p_registers.reg_eax = -1;
	  else if (proc_array[kill_pid].p_state == P_ZOMBIE)
	    {	
	      current->p_registers.reg_eax=0;
	    }
	  else{
	    if(proc_array[kill_pid].waiting_proc!=NULL)
	      {		
		proc_array[kill_pid].waiting_proc->p_state=P_RUNNABLE;
		proc_array[kill_pid].waiting_proc->p_registers.reg_eax=0;
		proc_array[kill_pid].p_exit_status=0;
		proc_array[kill_pid].p_state=P_EMPTY;
	      }
	    else
	      {
		proc_array[kill_pid].p_state=P_ZOMBIE;
		proc_array[kill_pid].p_exit_status=-1;
	      }	 
	    current->p_registers.reg_eax=0;
	  }
	  run(current);
	...
	case INT_SYS_WAIT: {
		...
		if (p <= 0 || p >= NPROCS || p == current->p_pid
		  ...
		  {	
		    ...
		    proc_array[p].waiting_proc=current;
		  }
		else
		  {		    
		    proc_array[p].p_state=P_EMPTY;
...
do_fork(process_t *parent)
{
  ...
  for(x;x<NPROCS;x++)
    {
      if(proc_array[x].p_state==P_EMPTY)
	{
	  empty_procID=x;
	  break;
	}
  ...
  process_t* proc=&proc_array[empty_procID];
  ...
  proc->p_state=P_RUNNABLE;
	...
	//                should arrange this.
	//   * ???????    There is one other difference.  What is it?  (Hin
...
copy_stack(process_t *dest, process_t *src)
{
	...
	dest_stack_top = src_stack_top+(dest->p_pid)*PROC_STACK_SIZE;
...
pid_t sys_newthread(void(*start_function)(void))
{
  ...
  proc_array[start_procID].p_state=P_RUNNABLE;
  proc_array[start_procID].p_registers.reg_eip=(int)start_function;
  proc_array[start_procID].p_registers.reg_eax=0; //return 0 for child
  return start_procID;
